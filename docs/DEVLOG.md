...
**待办（defer，不在 MVP）**：YAML 块间空行可读性；武侠等其余体裁语料；**PR10 大审查节点**（`/code-review`+`/security-review` 冷读 `1162e8d...<head>` 覆盖 PR9+PR10，含 chunker/示例 route 解析未信任输入）——用户拍板移到转 public 前（PR11）一起跑。

### PR10b 追加：真实多体裁语料 + 中文数字章节自然序修复（同日）

用户实测发现**多选 txt 顺序错乱**，并提供 7 份真实素材重整示例集。

- **根因（顺序 bug）**：红楼文件名是中文数字（`第三十六/七/八回`），`Intl.Collator(numeric)` 只认阿拉伯数字 → 按字符码位排（六/七/八 码位乱序）。修：`concatFiles` 加 `parseOrdinal`（阿拉伯+中文数字 一二三十百千两）+ `chapterOrdinal`（从文件名抽 `第N卷/章/回/节/部/篇`），先按回目序、再 collator 兜底。+3 测试（含红楼三回真实文件名）。
- **示例集换成 4 个真实体裁模块**：古典章回=红楼第36–38回（海棠诗社/菊花诗，文件名补回目 → 3 章）/ 现代网文=人生何处不青山+轮回秋日的信（同一小说连续 2 章合并，补 `第一章/第二章` 题）/ 散文=鲁迅《从百草园到三味书屋》/ 意识流=《追忆似水年华》节选。
