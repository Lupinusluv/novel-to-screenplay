...
- [x] **PR12 402 友好降级 + 本地部署引导 + 正式大审查** — ✅ **已并入 main（#23，merge commit `77de573`）**（分支 `pr12-graceful-402`，已删）。
  ① **402 → 结构化错误码友好降级**：`lib/llm/client.ts` 新增 `LLMError`(status/code) + `classifyLLMErrorCode`（402 + body 兜底 regex，输入限长 4096）+ `llmErrorCode`（走 `cause` 链）；`isRetryable` 显式把 LLMError 判 non-retryable。`events.ts`/`pipelineState.ts` 的 `error` 加 `code?`（过 SSE 无损）。`orchestrator.ts` `isFatalResourceError` 让余额耗尽从**任意 LLM 阶段**升级为 fatal 中止整条 run（不再吞成占位 + 假 `final_result`）、`runPool` 捕获首错停派重抛、`pipelineToSSEStream` 只对**致命** error 置 `sawFatalError`。`storyBible`/`sceneConverter`/`critic` 的 chatJSON wrap 加 `{ cause }`。`ConverterApp.tsx` 按 `code==="insufficient_balance"` 渲染友好块 + 本地部署链接（文案/链接常量在组件内守 E10）。
  ② **README 本地部署引导补强**：填真实仓库地址 + 额度耗尽指引对齐前端文案。
  ③ **正式大审查已过**（冷读 `git diff 1162e8d..HEAD` 覆盖 PR9–PR12，2 独立冷 reviewer + codex 设计冷读 SCORE 6/10）：**逮到并修 1 真 bug**（`sawError` 被非致命 scene 级 warning 置真 → 抑制带 code 的致命合成帧 → 前端永久卡 in-flight；修为 `sawFatalError` + 回归测试）+ **1 健壮性硬化**（分类 body regex 输入限长）。codex 两个 High（余额可能 mid-scenes 死 / LLMError 必须 non-retryable）设计阶段已吸收。
  **303 passed | 3 skipped**（+14）、`tsc`/`lint` 干净。设计 spec `docs/superpowers/specs/2026-06-07-pr12-graceful-402-design.md`；实现 + 大审查纪实见 `docs/DEVLOG.md` PR12 节。**注**：云端 ultra 大审查（`/code-review ultra`）为用户侧计费触发，本次为本地等价冷读。
- 以下为历史任务清单（PR12 三块，留作记录）：
...
> **当前状态快照（2026-06-07，PR12 合并后）**：**PR1–PR12 全部已并入 main**（HEAD=`77de573`，PR12=#23）。**仓库已转 public**。**功能 + 部署 + demo + 402 友好降级全收尾**：全栈端到端跑通、可编辑可溯源、吃脏输入、四体裁真示例、多 txt 自然序上传、视觉打磨、**余额耗尽友好降级**；**已上线 Render**（https://novel-to-screenplay-gtoh.onrender.com，端到端实跑验证）；demo 已录并填入 README（B 站 BV1ySEb6TEuC）；**用户已给 DeepSeek 充值**（402 当时是余额耗尽）。
> **PR12 已完成（见 §6 PR12 行）**：① **402 → 结构化错误码 `insufficient_balance` 友好降级**（HTTP 层归一、前端按码显示「演示站额度已用尽 + 本地部署引导 + GitHub 链接」、余额耗尽从任意 LLM 阶段升级为 fatal 中止整条 run）② **README 本地部署引导补强** ③ **正式大审查已过**（冷读 `1162e8d..HEAD` 覆盖 PR9–PR12，逮到并修 1 真 bug「sawError 抑制致命合成帧」+ 1 健壮性）。**项目所有计划内 PR 已收尾，剩运维（Render Suspend / 轮换 key）。**
> **现在全栈端到端跑通、可编辑可溯源、能吃脏输入、余额耗尽优雅降级**：小说 → chunk（**回目鲁棒识别 + 长度兜底 + 近重复检测 + ReDoS 行长闸**）→ StoryBible → 逐场景(convert+critic+自纠重试，**Critic 失败降级 needs_review 不中断；余额耗尽 fatal 中止**) → 汇编 → `POST /api/convert` SSE → **前端实时时间线 + 卡片(id 已解析中文名)/YAML + 溯源高亮原文 + YAML 回灌编辑 + 导出 + 402 友好提示**。
> main 上 `npm test` = **303 passed | 3 skipped**，`tsc` 干净，`lint` 干净；**线上 Render 端到端实跑验证过**（`/api/sample` 真实四体裁 + `/api/convert` SSE 四阶段 `final_result` 收尾，常驻容器跑完长流式不被掐）。
...
**接续步骤（PR1–PR12 全在 main，仓库已 public；下方为历史接续指引 + 剩余运维）**：
> 计划内开发已收尾。剩余仅**运维**（见步骤 6）。若再起新需求：设计走 gstack `/gstack-spec`+codex、实现走 superpowers TDD、branch-per-PR、用户点头才 merge。以下原 PR12 接续步骤留作流程范例：
