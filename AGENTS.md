...
**进度**：PR1（LLM client）/ PR2（Schema+YAML+SCHEMA.md）/ PR3（Chunker+红楼梦样本）/ PR4（StoryBible Curator）/ PR5（Scene Converter）/ PR6（Validator + Critic + Orchestrator + SSE）/ PR7（前端核心）/ PR8（溯源弹层 + YAML 回灌编辑 + 空错态打磨）/ PR9（切分鲁棒性·后端 chunker）/ PR10（前端打磨 MVP·视觉升级+多 txt 上传+四体裁示例集）/ **PR11（README + Render 部署 + demo 收尾 + 欠账大审查）** ✅ **全部已并入 main**。**项目功能层面已收尾、且已部署到 Render 线上可访问**（https://novel-to-screenplay-gtoh.onrender.com，端到端实跑验证过）。**现在全栈端到端跑通、可编辑可溯源、吃脏输入、四体裁真示例、多 txt 自然序上传、视觉打磨**：小说 → chunk（**回目鲁棒识别 + 长度兜底 + 近重复检测 + ReDoS 行长闸**）→ StoryBible → 逐场景(convert+critic+自纠重试，**Critic 失败降级 needs_review 不中断**) → 汇编 → `POST /api/convert` SSE → **前端实时时间线 + 卡片(id 解析中文名)/YAML + 溯源高亮原文 + YAML 回灌编辑 + 导出**。main 上 `npm test`=**289 passed | 3 skipped**、`tsc`/`lint` 干净；**线上 Render 端到端实跑验证过**（四体裁示例 + 长流式 `final_result` 收尾不被掐）。demo 视频已录并填入 README 顶部（B 站 BV1ySEb6TEuC）；用户已给 DeepSeek 充值。**PR12（下一个，用户 /clear 前拍板）= 402 余额耗尽前端友好提示（识别 Insufficient Balance→引导本地部署配自己 key）+ README 本地部署引导补强 + 正式云端大审查（锚 `1162e8d` 覆盖 PR9–PR12）**——基于新 main 开 `pr12-graceful-402`，完整任务清单与实现锚点见 `docs/PROJECT.md §6 PR12 行 + §10 接续步骤`。PR 路线图见 `docs/PROJECT.md §6`；实现纪实见 `docs/DEVLOG.md`（**尤其 PR11 节**）。

**质量门禁（强制，见 `docs/PROJECT.md §8.1`）**：每 PR 必跑 `npm test`+`npx tsc --noEmit`（贴原始输出）、TDD 先红、更新 `docs/DEVLOG.md`、用户点头才 merge。冷上下文大审查（`/code-review`+`/security-review`）每 2 PR 一次——**PR4、PR6、PR8 已审过；PR11 已跑欠账大审查**（锚 PR8 合并点 `1162e8d`，覆盖 **PR9+PR10+PR11**，逮到并修 1 个中危 ReDoS：`chunker.ts CHAPTER_HEADING` 加 `MAX_HEADING_LINE` 行长闸）。**PR9 当时只跑 codex 冷读（SCORE 7/10），PR10 用户拍板先合并、大审查并入 PR11——PR11 已做人工冷读并修了 1 个中危 ReDoS。PR12 = 用户要的正式云端大审查节点**（`/code-review`+`/security-review` 冷读 `1162e8d...<pr12-head>`，覆盖 PR9–PR12）。注：**doc 状态同步随 PR 一起合并，不单独直接 push main**（branch-per-PR；分类器会拦）。

**接续步骤（PR12 待开工，PR1–PR11 全在 main，已上线 Render）**：① 读 `docs/PROJECT.md`（**§6 PR12 行 = 完整任务清单与实现锚点 + §10 接续步骤**）+ `docs/DEVLOG.md`（**尤其 PR11 节**）+ `docs/SCHEMA.md`；② **`git checkout main && git pull` 然后 `git checkout -b pr12-graceful-402 main`**——别重做 PR1–PR11；③ **PR12 三块**：402 友好提示（识别 Insufficient Balance→引导本地部署，推荐归一成结构化错误码而非字符串匹配）/ README 本地部署引导补强 / 正式云端大审查（`/code-review`+`/security-review` 冷读 `git diff 1162e8d...<pr12-head>` 覆盖 PR9–PR12）；④ 设计走 gstack `/gstack-spec`+codex，实现走 superpowers TDD，用户点头才 merge；⑤ **评审/答辩交付后**（运维）：Render Suspend 服务 / 轮换 LLM key（公开链接烧用户自己的 key）。
