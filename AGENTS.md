...
**接续步骤（功能已收尾，PR1–PR11 全在 main，已上线 Render）**：① 读 `docs/PROJECT.md §10`（当前状态快照）+ `docs/DEVLOG.md`（**尤其 PR11 节**）+ `docs/SCHEMA.md` + `docs/DEMO-SCRIPT.md`；② **剩余唯一动作**：用户给出 demo 视频链接后，把 README 顶部 `Demo 视频：**<这里放 B 站/云盘链接>**` 换成真链接（branch-per-PR，别直推 main）；③ **评审交付后**：按需在 Render Suspend 服务 / 轮换 LLM key（公开链接烧用户自己的 key，余额耗尽 storybible 阶段会 402）；④ 若要加体裁：`samples/` 丢 `.txt` + `lib/samples/manifest.ts` 加一行。
